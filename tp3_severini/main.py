...
def modificar(pedido):
  ...
  for fila in lector_lista_pedidos:
    if fila[0] == pedido[NUMERO_PEDIDO]:
      if fila[1] == pedido[VERDURA_MODIFICAR] and frutas_venta(pedido[VERDURA_MODIFICAR]):
        if pedido[CANT_VERDURA_MODIFICAR] == "0":
          
          print("Este pedido fue eliminado ")
        elif pedido[CANT_VERDURA_MODIFICAR] != "0":
